...
class Outlying_Snow_Plains:
    ...
    async def teleport(self):
        ...
        await self.bot.switch_map(y_list=508/1080, world='jarilo_vi', scroll_down=False,
                                    x=998/2400, y=338/1080, corner='botright', move_x=0, move_y=0, confirm=True) # Long Slope
...
class Backwater_Pass:
    ...
    async def path_1(self):
        ...
        await self.bot.attack_technique(8, wait=False) # -1TP
...
class Great_Mine:
    ...
    async def path_1(self): # roamer
        ...
        await self.bot.movepi(0.4, 5000) # stability move
    async def path_2(self):
        logger_set_path(2)
    ...
    async def path_4(self): # roamer
        ...
        await self.bot.movepi(1.5, 500)
        await self.bot.attack_technique(2, wait=False) # -1TP
        await self.bot.position_fixing(1.25, 4000)
