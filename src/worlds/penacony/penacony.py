...
class Penacony:
    ...
    async def farm(self):
        ...
        Time    ???
        '''
        # await self.extra.restore_tp(tp=4, info='Penacony 0')    # TP:+4->5 Time:???
    ...
    async def dev(self):
        await self.dewlight_pavilion.farm()
        await self.golden_hour.restore_tp(tp=4.2)
        await self.penacony_grand_theater.farm()
        await self.audition_venue.farm()
        raise SystemExit()
